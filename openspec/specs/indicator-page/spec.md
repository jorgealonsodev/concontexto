...
Baseline capability specification — the source of truth for `indicator-page`.

Sources: PRD §6.1.1, §6.1.3, §12.2, §12.3, §14.2, §9.2, Anexo E.1, principles P1, P2, P4; ADR-7.
Contributing changes: `phase-1-indicator-page` (archived 2026-08-05) — established this capability.

**Language boundary.** This capability carries the product's first reader-facing copy. Every string a
...
The build MUST produce `/indicador/{slug}` for exactly these six slugs, which are **frozen** because
permalinks are a permanent commitment (Anexo E.1): `tasa-de-paro-epa`, `ocupados-epa`,
...
homepage deriving its own list would also be a second derivation of the route set.
...
six; those belong to later milestones.
