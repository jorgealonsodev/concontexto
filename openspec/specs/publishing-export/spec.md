# Spec: publishing-export

Baseline capability specification — the source of truth for `publishing-export`.

Sources: ADR-7, PRD §14.2, §9.2, §19.3, principle P5.
Contributing changes: `phase-1-indicator-page` (archived 2026-08-05) — established this capability.

Amendment history is deliberately not restated here. The `(Previously: …)` annotation this capability
carried while it was a delta — the narrowing of "An ingestion cycle exports what it learned, and only what
it learned", and the reasoning that produced it — is preserved verbatim in the archived change record at
`openspec/changes/archive/2026-08-05-phase-1-indicator-page/delta-specs/publishing-export/spec.md`, and in
that change's `design.md` under D-2.
...
The intent is that **a suspect datum must never be published**, not that the pipeline must go silent. The
publish gate already guarantees the former by writing no observation at all on a blocked run, so an export
after a validation failure publishes exactly the last valid data plus the honest fact that the latest run
failed. Suppressing it instead would serve a stale artifact that silently claimed everything was fine, and
would make `indicator-page`'s "The three page states of PRD §6.1.3" undeliverable, because a new artifact
is a reader's only route to that banner.
