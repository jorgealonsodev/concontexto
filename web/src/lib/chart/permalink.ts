...
import { CUSTOM_RANGE, type RangeSelection } from "../transform/sliceRange";
...
export function encodeChartState(state: ChartUrlState): string {
  ...
  const range = state.range === CUSTOM_RANGE && !custom ? DEFAULT_STATE.range : state.range;
...
export function decodeChartState(
  ...
  const state: ChartUrlState = {
    range: custom || selection !== CUSTOM_RANGE ? selection : DEFAULT_STATE.range,
    transform:
