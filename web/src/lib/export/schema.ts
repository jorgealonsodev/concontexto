 ...
 * `toEventRefs` additionally collapses an empty `noteMd` to nil before
 * marshalling, so the key is absent rather than `""` — hence `.min(1)`
 * under the `.optional()`, which rejects an empty string a drifted writer
 * would have to have gone out of its way to emit.
 */
