...
//
// The tests below are not a rendering checklist. Each one pins a property the
// mark must have in order NOT to make a claim the registry does not support:
// the owner asked for which measures were taken and when, and explicitly
// agreed the site must not say whether they worked — which is a constraint on
// the drawing before it is a constraint on the copy, because a rule crossing
// the data where the curve turns asserts an effect by adjacency, in every
// language, with no author.
describe("renderChartSVG — the policy-measure marks", () => {
  const MEASURES = [
    {
      id: "rdl-8-2020-covid-medidas-extraordinarias",
      group: "measures",
      name: "Real Decreto-ley 8/2020",
      dateStart: "2020-03-18",
    },
  ];
  const MEASURE_INPUT = { ...GOLDEN_INPUT, policyMeasures: MEASURES };

  /** The one `<g>` this layer emits, in the wide drawing. */
  function markGroup(svg: string): string | undefined {
    return /<g class="chart-measure-mark"[\s\S]*?<\/g>/.exec(svg)?.[0];
  }

  it("draws nothing at all when the reader has not opened the group", () => {
    expect(markGroup(renderChartSVG(GOLDEN_INPUT))).toBeUndefined();
  });

  it("draws one mark per measure inside the window, carrying the instrument's id", () => {
    const mark = markGroup(renderChartSVG(MEASURE_INPUT));
    expect(mark).toBeTruthy();
    expect(mark).toContain('data-measure-id="rdl-8-2020-covid-medidas-extraordinarias"');
  });

  it("names the instrument and its date of entry into force in the mark's own title", () => {
    const mark = markGroup(renderChartSVG(MEASURE_INPUT));
    expect(mark).toContain("Entrada en vigor");
    expect(mark).toContain("Real Decreto-ley 8/2020");
    expect(mark).toContain("18 de marzo de 2020");
  });

  it("is drawn ENTIRELY below the axis line — it never crosses the data", () => {
    // This is the load-bearing one. The plot area's bottom edge is the axis;
    // a mark whose y ever went above it would be standing on the series.
    const svg = renderChartSVG(MEASURE_INPUT);
    const axisY = Number(/class="chart-axis" x1="[\d.]+" y1="([\d.]+)"/.exec(svg)?.[1]);
    expect(Number.isFinite(axisY)).toBe(true);
    const mark = markGroup(svg) as string;
    const [, y1, y2] = /y1="([\d.]+)" x2="[\d.]+" y2="([\d.]+)"/.exec(mark) as RegExpExecArray;
    expect(Number(y1)).toBeGreaterThan(axisY);
    expect(Number(y2)).toBeGreaterThan(Number(y1));
  });

  it("borrows no other layer's encoding: no dash, no fill, not the provisional grey, not the government ink", () => {
    const mark = markGroup(renderChartSVG(MEASURE_INPUT)) as string;
    // The dash is reserved for provisional data (theme.css's own header) and
    // is unavailable to every annotation layer.
    expect(mark).not.toContain("stroke-dasharray");
    expect(mark).not.toContain("var(--color-provisional)");
    // A fill would make it a band; the break band is the only fill in this
    // drawing and it means "the two sides are not comparable".
    expect(mark).not.toContain("var(--color-break-band)");
    // And not the change-of-government ink. That layer is ALSO a vertical
    // mark at an instant, so ink plus verticality would leave region as the
    // single separation; the measure mark keeps its own colour as a second
    // channel behind the region, exactly as every other layer here does.
    expect(mark).not.toContain("var(--color-ink)");
    expect(mark).toContain("var(--color-event-span)");
  });

  it("carries no on-drawing label, so nothing names a measure beside the curve", () => {
    // Deliberate, and it is the one refusal in this layer. The government
    // rules and the event rails print their names inside the plot because
    // there is room there; a measure's name in the gutter would either be
    // drawn over the axis labels or over the data. The sentence beside the
    // chart, the chip below it and the legend carry the identification
    // instead — all of them away from the curve.
    const svg = renderChartSVG(MEASURE_INPUT);
    expect(svg).not.toContain("chart-annotation-label--measure");
  });

  it("renders the same layer in the narrow drawing", () => {
    const svg = renderChartSVG({
      ...MEASURE_INPUT,
      titleId: "golden-title-narrow",
      ...narrowChartVariant(GOLDEN_POINTS, 1),
    });
    expect(svg).toContain('data-testid="chart-measure-mark-narrow"');
  });
});
